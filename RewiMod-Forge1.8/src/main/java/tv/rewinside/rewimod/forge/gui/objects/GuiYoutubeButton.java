...
public class GuiYoutubeButton extends GuiButton implements IGuiButton {

	public GuiYoutubeButton(int buttonID, int x, int y) {
		super(buttonID, x, y, 20, 20, "");
	}

	@Override
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		CoreGuiDrawer.drawButtonYoutube(this, this.visible, this.xPosition, this.yPosition, mouseX, mouseY, this.width, this.height);

		if (CoordinateUtil.inbetween(xPosition, yPosition, mouseX, mouseY, width, height)) {
			this.drawString(mc.fontRendererObj, "Rewi's Kanal",
					this.xPosition + 24, this.yPosition + 6, Color.WHITE.getRGB());
		}
	}

	@Override
	public void onClick(int mouseButton) {
		CoreGuiActions.clickButtonYoutube();
	}
}
